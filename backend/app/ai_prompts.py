...
BACKLINKS_PROMPT = (
    ...
    "- `traffic_domain` and source-page `traffic` / `positions`: real "
    "traffic > vanity DR.\n"
    ...
    "weight it AFTER traffic — vanity DR with no traffic is a PBN tell.\n"
    ...
    "exact-match anchor stuffing, or PBN footprints (similar low-traffic "
    "high-DR sources with thin link profiles of their own).\n"
    "- 'high_quality' requires real DR + real traffic + natural anchor "
    "mix + snippets that read as genuine editorial mentions.\n\n"
    + CRITERION_JSON_SCHEMA
)
...
REFDOMAINS_PROMPT = (
    ...
    "- `traffic_domain` and `positions_source_domain`: source domains with "
    "real organic visibility are stronger than zero-traffic high-DR shells.\n"
