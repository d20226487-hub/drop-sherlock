...
const messagesEn = {
  ...
  pages: {
    ...
    // Strings shared verbatim by the Linked Domains Checker and SERP
    // Overview tool pages (run status panel, Recent-runs table + its
    // pagination). Kept in one place so both pages stay in sync.
    toolsShared: {
      // Run status panel
      runNo: (id: number) => `Run #${id}`,
      pause: "Pause",
      resume: "Resume",
      cancel: "Cancel",
      unitsLabel: "Ahrefs units:",
      unitsBilled: "billed",
      unitsListPrice: (n: string) => ` · ${n} list price`,
      apiCalls: (n: number) => ` · ${n} API call${n === 1 ? "" : "s"}`,
      // Recent-runs section
      recentRuns: "Recent runs",
      downloadAllDomains: "Download all unique domains (all runs)",
      copyAll: "Copy all",
      copyAllDone: (n: string) => `Copied ${n} ✓`,
      copyFailed: "Copy failed",
      searchPlaceholder: "Search by job name…",
      runsPage: (total: number, page: number, pages: number) =>
        `${total} run${total === 1 ? "" : "s"} · page ${page}/${pages}`,
      prev: "Prev",
      next: "Next",
      ...
      colName: "Name",
      colStatus: "Status",
      colAhrefsUnits: "Ahrefs units",
      colActions: "Actions",
      runFallbackName: (id: number) => `Run #${id}`,
    ...
    linkedDomains: {
      title: "Tool · Linked Domains Checker",
      description:
        "For each domain, pull the external domains it links out to via Ahrefs /linkeddomains (one call per domain). Runs as a persistent, resumable job — safe to leave running, and it survives restarts. Export the unique linked domains across all inputs as CSV. Only the domain column is fetched — 1 unit per returned domain, with a ~50 units/domain floor. Tip: domain ahrefs.com probes for free.",
      domainsLabel: "Domains (one per line · max 1000)",
      domainsPlaceholder: "example.com\nsomesite.org\nahrefs.com",
      parsedCount: (n: number) => `Parsed: ${n} domain${n === 1 ? "" : "s"}`,
      perTargetLimitLabel: "Per-target limit (optional)",
      perTargetLimitPlaceholder: "5000 (default)",
      skipCheckedLabel: "Skip already-checked domains",
      skipCheckedHint: "drop domains that already completed in a previous run",
      allowedTldsLabel: "Allowed TLDs only",
      allowedTldsHint:
        "fetch only linked domains whose TLD is in the Settings allowlist — filtered rows aren't billed",
      unitBudgetLabel: "Unit budget (optional)",
      unitBudgetPlaceholder: "auto-pause above N units",
      unitBudgetHint:
        "Pauses the run (resumable) once Ahrefs spend crosses this ceiling.",
      jobNameLabel: "Job name (optional)",
      jobNamePlaceholder: "auto-named from first domain",
      submit: "Run checker",
      submitting: "Submitting…",
      errAddDomain: "Add at least one domain",
      errMaxDomains: (n: number) => `Max 1000 domains per run (you have ${n})`,
      errPerTargetLimit: "Per-target limit must be between 1 and 5000",
      errUnitBudget: "Unit budget must be a positive number",
      errAllAlreadyChecked: (n: number) =>
        `All ${n} domain(s) were already checked in a previous run — nothing new to run.`,
      skippedBanned: (n: number, list: string, more: string) =>
        `Skipped ${n} banned domain(s): ${list}${more}`,
      skippedAlreadyChecked: (n: number, list: string, more: string) =>
        `Skipped ${n} already-checked domain${n === 1 ? "" : "s"} (from previous runs): ${list}${more}`,
      progressCounts: (
        processed: number,
        total: number,
        done: number,
        ...
        `${processed}/${total} targets · ${done} done · ${failed} failed · ${running} running · ${pending} pending`,
      downloadNewCsv: "Download NEW domains CSV",
      downloadAllRunCsv: "All run domains CSV",
      copyNew: "Copy new domains",
      copyNewDone: (n: number) => `Copied ${n} domains ✓`,
      colTargets: "Targets",
      colUniqueDomains: "Unique domains",
      newCsv: "New CSV",
      allCsv: "All CSV",
    },
    serpOverview: {
      title: "Tool · SERP Overview",
      description:
        "Bulk /serp-overview across a batch of keywords: the ranking-page URLs per keyword for each selected country (one run per country), limited to the top organic positions. Runs as a persistent, resumable job — survives restarts, and past runs stay downloadable below. Only the url column is fetched to keep spend at the ~50 units/keyword floor. Keywords already checked with the same country & result count are skipped by default for the window set in Settings → SERP Overview (30 days out of the box). Tip: keyword ahrefs or wordcount probes for free.",
      keywordsLabel: "Keywords (one per line · max 500)",
      keywordsPlaceholder: "купить телефон\nдоставка цветов алматы\nahrefs",
      parsedCount: (n: number) => `Parsed: ${n} keyword${n === 1 ? "" : "s"}`,
      countriesLabel: (n: number) => `Countries (${n} selected)`,
      countriesHint:
        "Each keyword is checked in each selected country — one run per country.",
      countrySearchPlaceholder: "Search countries…",
      noCountryMatches: "No matches.",
      selectedPrefix: "Selected:",
      clearCountries: "Clear",
      topPositionsLabel: "Top positions (1-100)",
      topPositionsPlaceholder: "10",
      unitBudgetLabel: "Unit budget (optional)",
      unitBudgetPlaceholder: "auto-pause above N units",
      jobNameLabel: "Job name (optional)",
      jobNamePlaceholder: "auto-named from first keyword",
      recheckLabel: "Recheck keywords",
      recheckHint:
        "check again even if a keyword was already checked with the same country & result count within the ignore window",
      submit: "Run SERP overview",
      submitting: "Submitting…",
      errAddKeyword: "Add at least one keyword",
      errMaxKeywords: (n: number) => `Max 500 keywords per run (you have ${n})`,
      errTopPositions: "Top positions must be between 1 and 100",
      errUnitBudget: "Unit budget must be a positive number",
      errSelectCountry: "Select at least one country",
      errMaxCountries: (n: number) =>
        `Max 30 countries per submit (you have ${n})`,
      errAllDuplicates: (windowDays: number, count: number) =>
        `All keywords in all selected countries were already checked within the last ${windowDays} days (${count} skipped) — tick "Recheck keywords" to force a re-check.`,
      dupNoticeCountry: (
        country: string,
        n: number,
        list: string,
        ...
        `${country}: skipped ${n} previously-checked keyword${n === 1 ? "" : "s"} (${list}${more})`,
      dupNoticeSkippedCountry: (country: string, n: number) =>
        `${country}: all ${n} keywords already checked — no run created`,
      multiNotice: (n: number, countries: string, first: string) =>
        `Created ${n} runs (${countries}) — viewing ${first}; open the others from Recent runs.`,
      progressCounts: (
        processed: number,
        total: number,
        done: number,
        ...
        `${processed}/${total} keywords · ${done} done · ${failed} failed · ${running} running · ${pending} pending`,
      downloadCsv: "Download CSV (keyword · position · URL)",
      downloadDomainsCsv: "Download unique domains CSV",
      copyUniqueDomains: "Copy unique domains",
      copyUniqueDomainsDone: (n: number) => `Copied ${n} domains ✓`,
      allowedTldsOnly: "Allowed TLDs only",
      colKeywords: "Keywords",
      colUrls: "URLs",
      downloadCsvShort: "Download CSV",
      domainsCsvShort: "Domains CSV",
    },
  },
};
...
const messagesRu: Messages = {
  ...
  pages: {
    ...
    toolsShared: {
      // Панель статуса запуска
      runNo: (id) => `Запуск #${id}`,
      pause: "Пауза",
      resume: "Продолжить",
      cancel: "Отменить",
      unitsLabel: "Единицы Ahrefs:",
      unitsBilled: "списано",
      unitsListPrice: (n) => ` · ${n} по прайсу`,
      apiCalls: (n) => ` · ${n} запрос(ов) к API`,
      // Раздел «Недавние запуски»
      recentRuns: "Недавние запуски",
      downloadAllDomains: "Скачать все уникальные домены (все запуски)",
      copyAll: "Копировать все",
      copyAllDone: (n) => `Скопировано ${n} ✓`,
      copyFailed: "Не удалось скопировать",
      searchPlaceholder: "Поиск по названию задачи…",
      runsPage: (total, page, pages) =>
        `Запусков: ${total} · страница ${page}/${pages}`,
      prev: "Назад",
      next: "Вперёд",
      noMatchingRuns: "Нет совпадений.",
      noRunsYet: "Запусков пока нет.",
      colDate: "Дата",
      colName: "Название",
      colStatus: "Статус",
      colAhrefsUnits: "Единицы Ahrefs",
      colActions: "Действия",
      runFallbackName: (id) => `Запуск #${id}`,
      failedSuffix: (n) => ` · ${n} с ошибкой`,
      view: "Открыть",
      rename: "Переименовать",
    ...
    linkedDomains: {
      title: "Инструмент · Проверка исходящих доменов",
      description:
        "Для каждого домена собирает внешние домены, на которые он ссылается, через Ahrefs /linkeddomains (один запрос на домен). Работает как постоянная задача с возможностью продолжения — можно оставить работать, переживает перезапуски. Выгрузите уникальные исходящие домены по всем входным данным в CSV. Забирается только колонка domain — 1 единица за каждый возвращённый домен, минимум ~50 единиц на домен. Совет: домен ahrefs.com проверяется бесплатно.",
      domainsLabel: "Домены (по одному в строке · макс. 1000)",
      domainsPlaceholder: "example.com\nsomesite.org\nahrefs.com",
      parsedCount: (n) => `Распознано: ${n} доменов`,
      perTargetLimitLabel: "Лимит на домен (необязательно)",
      perTargetLimitPlaceholder: "5000 (по умолчанию)",
      skipCheckedLabel: "Пропускать уже проверенные домены",
      skipCheckedHint:
        "исключить домены, уже завершённые в предыдущем запуске",
      allowedTldsLabel: "Только разрешённые TLD",
      allowedTldsHint:
        "забирать только исходящие домены, чей TLD есть в списке разрешённых в Настройках — отфильтрованные строки не тарифицируются",
      unitBudgetLabel: "Бюджет единиц (необязательно)",
      unitBudgetPlaceholder: "авто-пауза свыше N единиц",
      unitBudgetHint:
        "Ставит запуск на паузу (можно продолжить), когда расход Ahrefs превысит этот предел.",
      jobNameLabel: "Название задачи (необязательно)",
      jobNamePlaceholder: "имя по первому домену",
      submit: "Запустить проверку",
      submitting: "Отправка…",
      errAddDomain: "Добавьте хотя бы один домен",
      errMaxDomains: (n) => `Максимум 1000 доменов за запуск (у вас ${n})`,
      errPerTargetLimit: "Лимит на домен должен быть от 1 до 5000",
      errUnitBudget: "Бюджет единиц должен быть положительным числом",
      errAllAlreadyChecked: (n) =>
        `Все ${n} домен(ов) уже проверены в предыдущем запуске — запускать нечего.`,
      skippedBanned: (n, list, more) =>
        `Пропущено забаненных доменов: ${n}: ${list}${more}`,
      skippedAlreadyChecked: (n, list, more) =>
        `Пропущено уже проверенных доменов: ${n} (из предыдущих запусков): ${list}${more}`,
      progressCounts: (processed, total, done, failed, running, pending) =>
        `${processed}/${total} доменов · ${done} готово · ${failed} с ошибкой · ${running} в работе · ${pending} в очереди`,
      downloadNewCsv: "Скачать НОВЫЕ домены CSV",
      downloadAllRunCsv: "Все домены запуска CSV",
      copyNew: "Копировать новые домены",
      copyNewDone: (n) => `Скопировано ${n} доменов ✓`,
      colTargets: "Домены",
      colUniqueDomains: "Уникальные домены",
      newCsv: "Новые CSV",
      allCsv: "Все CSV",
    },
    serpOverview: {
      title: "Инструмент · Обзор SERP",
      description:
        "Массовый /serp-overview по набору ключевых слов: URL ранжирующихся страниц по каждому ключу для каждой выбранной страны (один запуск на страну), ограниченный топом органических позиций. Работает как постоянная задача с возможностью продолжения — переживает перезапуски, прошлые запуски остаются доступны для скачивания ниже. Забирается только колонка url, чтобы удержать расход на минимуме ~50 единиц на ключ. Ключи, уже проверенные с той же страной и числом результатов, по умолчанию пропускаются в течение окна, заданного в Настройках → Обзор SERP (из коробки 30 дней). Совет: ключ ahrefs или wordcount проверяется бесплатно.",
      keywordsLabel: "Ключевые слова (по одному в строке · макс. 500)",
      keywordsPlaceholder: "купить телефон\nдоставка цветов алматы\nahrefs",
      parsedCount: (n) => `Распознано: ${n} ключей`,
      countriesLabel: (n) => `Страны (выбрано ${n})`,
      countriesHint:
        "Каждый ключ проверяется в каждой выбранной стране — один запуск на страну.",
      countrySearchPlaceholder: "Поиск стран…",
      noCountryMatches: "Нет совпадений.",
      selectedPrefix: "Выбрано:",
      clearCountries: "Очистить",
      topPositionsLabel: "Топ позиций (1-100)",
      topPositionsPlaceholder: "10",
      unitBudgetLabel: "Бюджет единиц (необязательно)",
      unitBudgetPlaceholder: "авто-пауза свыше N единиц",
      jobNameLabel: "Название задачи (необязательно)",
      jobNamePlaceholder: "имя по первому ключу",
      recheckLabel: "Перепроверить ключи",
      recheckHint:
        "проверить заново, даже если ключ уже проверялся с той же страной и числом результатов в течение окна пропуска",
      submit: "Запустить обзор SERP",
      submitting: "Отправка…",
      errAddKeyword: "Добавьте хотя бы одно ключевое слово",
      errMaxKeywords: (n) => `Максимум 500 ключей за запуск (у вас ${n})`,
      errTopPositions: "Топ позиций должен быть от 1 до 100",
      errUnitBudget: "Бюджет единиц должен быть положительным числом",
      errSelectCountry: "Выберите хотя бы одну страну",
      errMaxCountries: (n) => `Максимум 30 стран за отправку (у вас ${n})`,
      errAllDuplicates: (windowDays, count) =>
        `Все ключи во всех выбранных странах уже проверены за последние ${windowDays} дн. (пропущено ${count}) — отметьте «Перепроверить ключи», чтобы проверить принудительно.`,
      dupNoticeCountry: (country, n, list, more) =>
        `${country}: пропущено ранее проверенных ключей: ${n} (${list}${more})`,
      dupNoticeSkippedCountry: (country, n) =>
        `${country}: все ${n} ключей уже проверены — запуск не создан`,
      multiNotice: (n, countries, first) =>
        `Создано запусков: ${n} (${countries}) — открыт ${first}; остальные откройте в разделе «Недавние запуски».`,
      progressCounts: (processed, total, done, failed, running, pending) =>
        `${processed}/${total} ключей · ${done} готово · ${failed} с ошибкой · ${running} в работе · ${pending} в очереди`,
      downloadCsv: "Скачать CSV (ключ · позиция · URL)",
      downloadDomainsCsv: "Скачать уникальные домены CSV",
      copyUniqueDomains: "Копировать уникальные домены",
      copyUniqueDomainsDone: (n) => `Скопировано ${n} доменов ✓`,
      allowedTldsOnly: "Только разрешённые TLD",
      colKeywords: "Ключи",
      colUrls: "URL",
      downloadCsvShort: "Скачать CSV",
      domainsCsvShort: "Домены CSV",
    },
  },
};
